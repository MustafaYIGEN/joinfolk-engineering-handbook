...
| PAYMENT_AUTHORITY_MARK_ORDER_PAID_V1 | APPLIED_AND_VERIFIED / CLOSED | Client execution of `public.mark_order_paid_v1(uuid)` was contained by platform commit `35dc24ba` and migration `20260716013000_p0_mark_order_paid_client_containment`; production final ACL is PUBLIC=false, anon=false, authenticated=false, service_role=true explicit. Function body remained unchanged and exact remote migration history was verified. |
...
| PURCHASE_RPC_CANONICALIZATION | BLOCKER | Legacy `v3`, both `v4` overloads, and `v5` client execution are closed separately. Overall canonicalization remains blocked by the open replay/idempotency, inventory concurrency, section-price authority, gift-contract parity, and remaining purchase-authority gates listed below. |
| PURCHASE_LEGACY_DIRECT_ISSUANCE_SURFACE | APPLIED_AND_VERIFIED / CLOSED | Commit `aff0c2a` removed the active v3 fallback; canonical migration `20260716013200_p0_purchase_legacy_client_containment` narrowed legacy direct issuance to service_role and retained authenticated/service_role commerce order creation. Production dry-run, postcondition, history reconciliation, reserved-seat and standing UAT passed. |
| COMMERCE_ISSUANCE_REPLAY_GAP | P0 / OPEN | Separate from legacy client containment. |
| INVENTORY_RACE_RISK | P0 / OPEN | Separate from legacy client containment. |
| SECTION_PRICE_AUTHORITY_GAP | OPEN | Separate from legacy client containment. |
| GIFT_COMMERCE_CONTRACT_DRIFT | OPEN | Separate from legacy client containment. |
