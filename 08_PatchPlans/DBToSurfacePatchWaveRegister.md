...
| --- | --- | --- | --- |
| DBSURF-P0-PAYMENT-AUTHORITY | remaining payment authority review: `create_commerce_order_v1`, `confirm_order_payment_v1`, `_issue_tickets_from_order_v1`, `expire_stale_orders_v1`; `mark_order_paid_v1` containment is closed separately | `mark_order_paid_v1`: canonical platform commit `35dc24ba`, migration `20260716013000_p0_mark_order_paid_client_containment`, remote history present, final client-deny/service-role-explicit ACL verified, body unchanged, rollback not required; remaining functions retain their own review preconditions | BLOCKED for remaining scope; `PAYMENT_AUTHORITY_MARK_ORDER_PAID_V1` is APPLIED_AND_VERIFIED / CLOSED |
| DBSURF-P0-CHECKIN-COMPAT | legacy `check_in_ticket`, `checkin_ticket_v2`, `checkin_ticket_by_id_v2`, unsafe wrappers | legacy client execution contained by `745b5846` / `20260716013100_p0_legacy_checkin_client_containment`; body unchanged, explicit service_role EXECUTE preserved, migration history aligned | APPLIED_AND_VERIFIED / CLOSED |
| DBSURF-P0-PURCHASE-LEGACY-DIRECT-ISSUANCE | `purchase_event_ticket_v3`, both `purchase_event_ticket_v4` overloads, and `purchase_event_ticket_v5`; active mobile v3 fallback | commits `b7e49aad` and `b1ff8d12`; migration `20260716013200_p0_purchase_legacy_client_containment` applied with production dry-run/postcondition and history aligned; `aff0c2a` routes active buyer mutation through `createOrder` / `create_commerce_order_v1`; bodies, MD5s, SECURITY DEFINER, and search_path unchanged | APPLIED_AND_VERIFIED / CLOSED |
