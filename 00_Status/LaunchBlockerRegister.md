...
| PURCHASE_RPC_CANONICALIZATION | BLOCKER | Legacy `v3`, both `v4` overloads, and `v5` client execution are closed separately. Overall purchase canonicalization remains blocked by `COMMERCE_ISSUANCE_REPLAY_GAP`, `INVENTORY_RACE_RISK`, `SECTION_PRICE_AUTHORITY_GAP`, `GIFT_COMMERCE_CONTRACT_DRIFT`, and remaining canonical purchase-authority decisions. |
