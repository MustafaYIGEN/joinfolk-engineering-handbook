...
| `public.check_in_ticket(text, uuid)` | contained legacy surface; historical PostgREST usage and separate `event_tickets` model remain | `DEPRECATE_OBSERVE_CANDIDATE` | client EXECUTE containment is verified; function/table are retained and not claimed dead | separate dependency, data-retention, and pruning decision; do not reopen `CHECKIN_LEGACY_COMPATIBILITY` merely because legacy objects remain |
...
| `public.mark_order_paid_v1(uuid)` | live SECURITY DEFINER function, contained to explicit service_role EXECUTE only; exact body MD5 `5cbb9851788530daf956a0b283e581b3` unchanged | `DEPRECATE_OBSERVE_CANDIDATE` | containment closes the client payment-authority bypass but does not approve removal or establish a canonical replacement | separate removal/canonicalization decision with dependency and observation review |
