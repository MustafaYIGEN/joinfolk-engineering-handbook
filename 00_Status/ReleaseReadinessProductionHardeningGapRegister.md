...
| RR-GAP-024 | AUTH-EMAIL-01 | Auth email mobile-first contract audit | Auth email / universal links / fallback routing | Browser-first target host is now decided as `join-folk.com`, but the canonical web reset route still renders the wrong surface, browser reset completion still needs implementation, `app.join-folk.com` remains rejected as the current auth email host, and rollout-safe cleanup remains outstanding. | Mixed live/source/handbook evidence; browser-first contract is accepted, but rollout/UAT closure remains active | Security-sensitive, product correctness | Candidate P1 | Pre-launch hardening candidate; Patch plan authorized; Production verification required | Browser reset implementation, UAT, and cleanup proof | Execute the accepted browser-first AUTH-EMAIL patch plan in rollout-safe order | PP-11 |
