...
| RR-GAP-024 | AUTH-EMAIL-01 | Auth email mobile-first contract audit | Auth email / universal links / fallback routing | Browser-first target host is decided as `join-folk.com` and the canonical web reset implementation is deployed, but token-bearing browser reset UAT, legacy native recovery regression UAT, normal onboarding regression UAT, and rollout-safe cleanup remain open. | Mixed live/source/handbook evidence; browser-first contract is accepted, implementation is deployed, and rollout/UAT closure remains active | Security-sensitive, product correctness | Candidate P1 | Pre-launch hardening candidate; Patch plan authorized; Production verification required | Browser reset UAT, mobile regression UAT, and cleanup proof | Execute the accepted browser-first AUTH-EMAIL patch plan in rollout-safe order | PP-11 |
...
- PP-11 Auth Email Browser-First Closure Pack: canonical browser auth host, browser reset completion, browser confirmation completion, rollout-safe legacy reset compatibility, and redirect/template cleanup alignment.
