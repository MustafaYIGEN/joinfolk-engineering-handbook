...
- Source confidence: Production ACL export + confirmed runtime evidence through `08f` + confirmed family evidence through `09` + confirmed priority contracts through `10c` + read-only local static caller inspection
- canonical: false
...
| P0 | live `public.dm_*` RPC family | Current live grants include anon despite authenticated dashboard usage, authenticated table policies, and high runtime activity | `10d` through `10g` remain pending; recovered local bodies are still incomplete | search all dashboard, mobile, web, Edge, and backend callers | inspect persona scope, participant checks, and message mutation authorization in each body | active family observed; `dm_archive_conversation_v1` specifically not observed | BLOCKED | full DM RPC ACL rollback required before any later change |
